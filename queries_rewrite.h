...
namespace Trinity
{
        ...
        static auto run_next(simple_allocator &allocator, const std::vector<ast_node *> &run, const uint32_t i, const uint8_t maxSpan, L &&l)
        {
                ...
                static thread_local std::vector<std::pair<str32_t, uint8_t>> v;
                static thread_local simple_allocator altAllocatorInstance;
                static thread_local std::vector<str32_t> alts;
                ...
                v.clear();
                v.push_back({{token.data(), uint32_t(token.size())}, 1});
                altAllocator.reuse();
                ...
                for (const auto *p = v.data(), *const e = p + v.size(); p != e;)
                {
                        ...
                        do
                        {
                                alts.push_back(p->first);
                        } while (++p != e && p->second == span);

                        if (alts.size() > 1)
                        {
                                auto lhs = ast_parser(alts.front(), allocator).parse();
                                ...
                                for (uint32_t i{1}; i != alts.size(); ++i)
                                {
                                        ...
                                        n->binop.lhs = lhs;
                                        n->binop.rhs = ast_parser(alts[i], allocator).parse();
                                        lhs = n;
                                }
                        ...
                        {
                                node = Trinity::ast_parser(alts.front(), allocator).parse();
                        }
	...
	Trinity::rewrite_query(inputQuery, 3, [](const auto tokens, const auto cnt, auto &allocator, auto out)
	{
		if (cnt == 1)
		{
			if (tokens[0].Eq(_S("PS4"))
				out->push_back("PLAYSTATION AND 4");
		}
		else if (cnt == 2)
		{
			if (tokens[0].Eq(_S("MAC")) && tokens[1].Eq(_S("BOOK")))
				out->push_back("MACBOOK");
		}
		else if (cnt == 3)
		{
