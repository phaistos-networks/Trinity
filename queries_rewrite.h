#ifndef _HOME_SYSTEM_DEVELOPMENT_PROJECTS_TRINITY_QUERIES_REWRITE_H
#define _HOME_SYSTEM_DEVELOPMENT_PROJECTS_TRINITY_QUERIES_REWRITE_H
...
// be extremely complex, many 100s of nodes in size, so you need to compromise and accept tradeoffs.
#pragma once
#include "queries.h"
...
namespace Trinity {
        ...
        struct flow final {
                ...
                ast_node *materialize(simple_allocator &a) const {
                        [[maybe_unused]] static ast_node dummy{.type = ast_node::Type::Dummy};
                        ...
                        if (const auto n = ents.size()) {
                                ...
                                return lhs ?: &dummy;
                                //return lhs ?: ast_node::make(a, ast_node::Type::Dummy);
                        }

                        return &dummy;
                        //return ast_node::make(a, ast_node::Type::Dummy);
                }
        ...
        static auto run_next(std::size_t &budget, query &q, const std::vector<ast_node *> &run, const uint32_t i, const uint8_t maxSpan, L &&l, gen_ctx &genCtx) {
                ...
                auto &                                                                                      altAllocator{altAllocatorInstanceTLS};
		...
		static constexpr bool inherit_app_phrase_id{true};
                ...
                // TODO(markp): consider this implementation.
                //
        ...
        static std::pair<ast_node *, uint8_t> run_capture(const uint32_t rewriteFlags, std::size_t &budget, query &q, const std::vector<ast_node *> &run, const uint32_t i, L &&l, const uint8_t maxSpan, gen_ctx &genCtx) {
                ...
                // TODO(markp): we can probably avoid using std::unordered_set<> here which would make sense, but let's consider alternatives later
                [[maybe_unused]] const auto common_anchestor = [&](flow **const flows, const size_t cnt, const bool oneFlowUseParent = false) -> flow * {
                        ...
                        for (size_t i{1}; i != penultimate; ++i) {
                                flows[i]->validate();
