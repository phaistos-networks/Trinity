...
#if 0 // doens't work yet, because in down() k can be > size_ +1 
	// This variant eliminates at least 2 cmps/jumps
        template <typename T, class Compare = std::less<T>>
        class priority_queue_with_sentinel
        {
              public:
                using const_reference = const T &;
                using reference = T &;
		...
		const T sentinel;
			...
			require(heap[size_+ 1]  == sentinel);

                        if (cmp(heap[k], heap[j]))
                        {
                                // right child exists and smaller than left
                                j = k;
                        }

                        while (cmp(heap[j], node))
                        {
                                heap[i] = std::move(heap[j]); // shift up child
                                i = j;
                                j = i << 1;
              ...
              public:
                priority_queue_with_sentinel(const uint32_t m, T s)
                    : capacity_{m}, heap((T *)malloc(sizeof(T) * (0 == capacity_ ? 3 : capacity_ + 2))), sentinel{s} // base-1 based, heap[0] is not used
                {
                        if (unlikely(!heap))
                                throw Switch::data_error("Failed to allocate memory");
                ...
                bool try_push(const T v, T &prev)
                {
                        ...
                        else if (size_ && !Compare{}(v, heap[1]))	 // need to check if (size_t) because a queue can be empty
                        {
                                prev = std::move(heap[1]);
                                heap[1] = std::move(v);
                                update_top();
