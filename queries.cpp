        ...
        for (;;) {
			...
			if (traceParser) {
				SLog("Consumed token of lengh ", pair.first, " => [", str_view32(ctx.lastParsedToken, pair.second), "]\n");
                        ...
                        while (!ctx.content.empty() && ctx.content.front() == '-')
                                ctx.content.strip_prefix(1);

                        if (unlikely(pair.second > Trinity::Limits::MaxTermLength)) {
                                // TODO(markp): what's the right thing to do here?
                                return {{}, {}};
                        } else {
                ...
                if (ctx.content.empty() || (!ctx.groupTerm.empty() && strchr(ctx.groupTerm.back(), ctx.content.front()))) {
                        return {{}, {}};
                } else if (ctx.content) {
...
static ast_node *parse_phrase_or_token(ast_parser &ctx) {
        ...
        if (ctx.content && ctx.content.StripPrefix(_S("\""))) {
                ...
                if (!n)
                        ...
                        auto p    = static_cast<phrase *>(ctx.allocator.Alloc(sizeof(phrase) + sizeof(term) * n));
                ...
                auto p    = static_cast<phrase *>(ctx.allocator.Alloc(sizeof(phrase) + sizeof(term)));
...
static std::pair<Operator, uint8_t> parse_operator_impl(ast_parser &ctx) {
        ...
        if (0 == (ctx.parserFlags & uint32_t(ast_parser::Flags::ANDAsToken)) && try_strip_token(s, "AND"_s32)) {
                ...
                switch (f) {
                        ...
                        case '+':
                                if (char_t c; s.size() > 1 && (c = s.p[1]) && !isspace(c) && c != '+') {
                                        // TODO(markp): this should have been a bit more sophisticated
                ...
                if (!ctx.groupTerm.empty() && strchr(ctx.groupTerm.back(), f))
...
// another benefit: string interning for quick checks
...
static void assign_query_indices(ast_node *const n, query_assign_ctx &ctx) {
        if (n->is_unary()) {
                ...
                // index perhaps?
                // TODO(markp): does this make sense?
                for (size_t i{0}; i != n->match_some.size; ++i) {
                        auto it = n->match_some.nodes[i];
